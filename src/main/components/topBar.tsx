...
const playAlertSound = async (buttonKey: string, settings: SettingsType) => {
  ...
  if (customPath) {
    try {
      const src = await window.electron.readSoundFileAsDataUrl(customPath);
      if (!src) {
        console.error(`[Sound] Could not read custom sound file for "${buttonKey}":`, customPath);
...
const TopBar = (props: Props) => {
  ...
  const pendingStateRef = React.useRef<Record<string, PendingState>>(
    Object.fromEntries(PENDING_KEYS.map((k) => [k, { count: 0, acknowledgedCount: 0 }]))
  );
  ...
  React.useEffect(() => {
    const applyCounts = (counts: Record<string, number>, dispatcherPresent: Record<string, boolean>) => {
      ...
      const nextPendingState: Record<string, PendingState> = {};

      // All computed here, outside any React state updater, so it runs
      // exactly once per call with no risk of React re-invoking this logic
      // and mutating wasFlashingRef more than once for the same update -
      // that mismatch was silently suppressing the sound on every call
      // after the first, while the flash (driven by plain state) stayed
      // unaffected.
      for (const key of PENDING_KEYS) {
        const newCount = counts[key] ?? 0;
        const prevState = pendingStateRef.current[key] ?? { count: 0, acknowledgedCount: 0 };
        // Reset acknowledgement once the channel is empty, so the next
        // arrival always triggers a fresh alert instead of being silently
        // suppressed by a stale acknowledgedCount left over from a
        // previous, already-handled call.
        ...
        nextPendingState[key] = { count: newCount, acknowledgedCount };
  ...
  const acknowledgePending = (buttonKey: string) => {
    setPending((prev) => {
      ...
      const updated = { ...state, acknowledgedCount: state.count };
      pendingStateRef.current = { ...pendingStateRef.current, [buttonKey]: updated };
      wasFlashingRef.current[buttonKey] = false;
      return { ...prev, [buttonKey]: updated };
    });
  };
    ...
    <Box>
      ...
      >
        <Stack direction="row" spacing={1} alignItems="center">
          ...
          <Tooltip title={props.editMode ? "Done editing layout" : "Edit layout"}>
            <IconButton
              size="small"
              onClick={props.onToggleEditMode}
              sx={{
                color: props.editMode ? "#fff" : "text.secondary",
                backgroundColor: props.editMode ? "warning.main" : "transparent",
                "&:hover": {
                  backgroundColor: props.editMode ? "warning.main" : "action.hover",
                  filter: props.editMode ? "brightness(1.1)" : "none",
                },
              }}
            >
