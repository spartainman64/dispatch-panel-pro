...
class TS3ClientQuery {
  ...
  async getAllCivilians(
    ...
    if (!profile?.civilianChannelId && !profile?.civilianBusinessChannelId) return [];

    const relevantChannelIds = [profile.civilianChannelId, profile.civilianBusinessChannelId].filter(
      (id): id is string => !!id
    );

    return clients
      .filter((c) => relevantChannelIds.includes(c.channelId))
